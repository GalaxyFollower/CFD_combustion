...
syms C_etae C_eta C_etaw real

% Define inner Temperatures as interpolation of outer Temperatures
T_sw=(T_SW+T_S+T_P+T_W)/4;%not used
T_se=(T_S+T_SE+T_E+T_P)/4;%not used
...
C_sw=(C_SW+C_S+C_P+C_W)/4;%not used
C_se=(C_S+C_SE+C_E+C_P)/4;%not used
C_ne=(C_P+C_E+C_NE+C_N)/4;
C_nw=(C_W+C_P+C_N+C_NW)/4;

C_s= (C_S+C_P)/2;%not used
C_e=(C_E+C_P)/2;
C_n=(C_N+C_P)/2;
C_w=(C_W+C_P)/2;
...
% Build hole stecil acounting for quadratic lambda like in Helmholtz 
  ...
  DDT =( C_etae^2*dy_e_ne*dTdx_etae - C_etae^2*dx_e_ne*dTdy_etae  ...
      +  C_n^2*dy_ne_nw*dTdx_n   - C_n^2*dx_ne_nw*dTdy_n ...
      +  C_etaw^2*dy_nw_w*dTdx_etaw - C_etaw^2*dx_nw_w*dTdy_etaw)/S_eta;
